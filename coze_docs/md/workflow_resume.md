https://www.coze.cn/open/docs/developer_guides/workflow_resume
...
恢复运行已中断的工作流。​
接口说明​
当通过[执行工作流（流式响应）](https://www.coze.cn/docs/developer_guides/workflow_stream_run)执行包含问答节点的工作流时，智能体会以指定问题向用户提问，并等待用户回答。此时工作流为中断状态，开发者需要调用此接口回答问题，并恢复运行工作流。如果问答节点勾选了从回复中提取字段，当用户的响应和智能体预期提取的信息不匹配，例如缺少必选的字段，或字段数据类型不一致，工作流会再次中断并追问。如果询问 3 次仍未收到符合预期的回复，则判定为工作流执行失败。​
[流式恢复运行工作流](https://www.coze.cn/open/docs/developer_guides/workflow_resume)和[恢复运行工作流](https://www.coze.cn/open/docs/developer_guides/resume_workflow) 的区别如下：​
  * 如果调用[执行工作流（流式响应）](https://www.coze.cn/docs/developer_guides/workflow_stream_run)API 时工作流中断，恢复时需要使用[恢复运行工作流（流式响应）](https://www.coze.cn/open/docs/developer_guides/workflow_resume) API，该 API 通过流式返回执行结果。​


  * 如果调用[执行工作流](https://www.coze.cn/open/docs/developer_guides/workflow_run) API 时工作流中断，恢复时需要使用[恢复运行工作流](https://www.coze.cn/open/docs/developer_guides/resume_workflow) API，该 API 支持同步运行或异步运行返回执行结果。​
...
限制说明​
  ...
  * 恢复运行工作流也会产生 token 消耗，且与[执行工作流（流式响应）](https://www.coze.cn/docs/developer_guides/workflow_stream_run)时消耗的 token 数量相同。​
...
权限​| run​确保调用该接口使用的访问令牌开通了 run 权限，详细信息参考[鉴权方式](https://www.coze.cn/docs/developer_guides/authentication)。​  
接口说明​| 恢复运行已中断的工作流。​  
​
请求参数​
Header​
...
event_id​| String​| 必选​| 74048319882025***​| 工作流执行中断事件 ID。你可以从[执行工作流（流式响应）](https://www.coze.cn/docs/developer_guides/workflow_stream_run)的响应信息中获得中断事件 ID。​  
interrupt_type​| Integer​| 必选​| 2​| 中断类型，你可以从[执行工作流（流式响应）](https://www.coze.cn/docs/developer_guides/workflow_stream_run)的响应信息中获得中断事件的中断类型。​  
resume_data​| String​| 必选​| 杭州，2024-08-20​| 恢复执行时，用户对智能体指定问题的回复。​如果是问答节点导致的中断，回复中应包含问答节点中的必选参数，否则工作流会再次中断并提问。​如果要传入 Image 等类型的文件，可以调用[上传文件](https://www.coze.cn/open/docs/developer_guides/upload_files)​上传文件API 获取 file_id，在调用此 API 时在 resume_data 中以序列化之后的 JSON 格式传入 file_id。例如 “resume_data” : "{\"file_id\": \"1456234***\"}"。​  
​
返回参数​
​
说明
...
interrupt_data​| Object of Interrupt​| {"data":"","event_id":"74048319882***/63020599195167***","type":2}​| 中断事件的详细信息，包含中断控制内容、中断类型和事件 ID 等信息。​  
ext​| JSON Map​| ​| 额外字段。​  
...
usage​| Object of Usage​| ​| 资源使用情况​  
detail​| Object of ResponseDetail​| 20241210152726467C48D89D6DB2****​| 响应详情。​  
...
参数​| 类型​| 示例​| 说明​  
---|---|---|---  
data​| String​| {"interrupt_data":{"data":"","event_id":"7404830425073352713/2769808280134765896","type":2},"node_title":"问答"}​| 中断控制内容。​  
type​| Integer​| 2​| 工作流中断类型，你可以从[执行工作流（流式响应）](https://www.coze.cn/docs/developer_guides/workflow_stream_run)的响应信息中获得中断时间的中断类型。​枚举值：​
  * 1：端插件触发中断。​
...
event_id​| String​| 740483198820252***​| 工作作流中断事件 ID，恢复运行时应回传此字段。​  
​
Usage​
​
参数​| 类型​| 示例​| 说明​  
...
示例
错误码
![](https://bat.bing.com/action/0?ti=97151251&Ver=2&mid=f6afdcb6-8f2d-4fe2-b8fc-ea3be2f0277a&bo=1&sid=a6c3ee30d4c211f0966da1ab2d8de18c&vid=a6c3dd00d4c211f0a7ba75271dd5e1e5&vids=0&msclkid=N&uach=pv%3D19.0.0&pi=0&lg=zh-CN&sw=1280&sh=720&sc=24&nwd=1&tl=%E6%89%A3%E5%AD%90%20-%20AI%20Agent%E6%99%BA%E8%83%BD%E5%8A%9E%E5%85%AC%E5%B9%B3%E5%8F%B0%20-%20%E6%89%A3%E5%AD%90%E7%94%A8AI%E9%87%8D%E5%A1%91%E7%94%9F%E4%BA%A7%E5%8A%9B%E4%B8%8E%E5%B7%A5%E4%BD%9C%E6%95%88%E7%8E%87&kw=%E6%89%A3%E5%AD%90,%20Coze,%20AI%20Agent,%20%E6%99%BA%E8%83%BD%E5%8A%9E%E5%85%AC,%20%E5%8A%9E%E5%85%AC%E6%8F%90%E6%95%88,%20AI%E5%86%99%E4%BD%9C,%20PPT%E7%94%9F%E6%88%90,%20%E7%BD%91%E9%A1%B5%E5%BC%80%E5%8F%91,%20%E7%94%9F%E4%BA%A7%E5%8A%9B%E5%B7%A5%E5%85%B7&p=https%3A%2F%2Fwww.coze.cn%2Fopen%2Fdocs%2Fdeveloper_guides%2Fworkflow_resume&r=&mtp=10&evt=pageLoad&sv=2&cdb=AQAQ&rn=891000)
![](https://bat.bing.com/action/0?ti=97179855&Ver=2&mid=5f557eab-910a-471a-ac00-9b6accd109d7&bo=1&sid=a6c3ee30d4c211f0966da1ab2d8de18c&vid=a6c3dd00d4c211f0a7ba75271dd5e1e5&vids=0&msclkid=N&uach=pv%3D19.0.0&pi=0&lg=zh-CN&sw=1280&sh=720&sc=24&nwd=1&tl=%E6%89%A3%E5%AD%90%20-%20AI%20Agent%E6%99%BA%E8%83%BD%E5%8A%9E%E5%85%AC%E5%B9%B3%E5%8F%B0%20-%20%E6%89%A3%E5%AD%90%E7%94%A8AI%E9%87%8D%E5%A1%91%E7%94%9F%E4%BA%A7%E5%8A%9B%E4%B8%8E%E5%B7%A5%E4%BD%9C%E6%95%88%E7%8E%87&kw=%E6%89%A3%E5%AD%90,%20Coze,%20AI%20Agent,%20%E6%99%BA%E8%83%BD%E5%8A%9E%E5%85%AC,%20%E5%8A%9E%E5%85%AC%E6%8F%90%E6%95%88,%20AI%E5%86%99%E4%BD%9C,%20PPT%E7%94%9F%E6%88%90,%20%E7%BD%91%E9%A1%B5%E5%BC%80%E5%8F%91,%20%E7%94%9F%E4%BA%A7%E5%8A%9B%E5%B7%A5%E5%85%B7&p=https%3A%2F%2Fwww.coze.cn%2Fopen%2Fdocs%2Fdeveloper_guides%2Fworkflow_resume&r=&mtp=10&evt=pageLoad&sv=2&cdb=AQAQ&rn=494224)
rangeDom